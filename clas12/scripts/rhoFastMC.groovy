...
class StraightLine {
    ...
    StraightLine(Particle myParticle) {
        this.myParticle = myParticle
        getStartPoint(myParticle)
    }

    void getStartPoint(Particle particle){
...
class Detector {
    ...
    Detector(String name, ArrayList<Shape3D> components) {
        this.components = components
    }

    void addComponent(Shape3D shape){
        this.components.add(shape)
    }

    void removeComponent(int sector){
        this.components.remove(sector)
    }

    void removeComponent(Shape3D shape){
        this.components.remove(shape)
    }

    Shape3D getComponent(int sector){
        return this.components.get(sector)
    }

    boolean hasIntersection(Line3D line){
    ...
    int intersection(Line3D line, ArrayList<Point3D> point) {
        Iterator<Shape3D> iter = components.iterator()
        int count = 0
        while (iter.hasNext()) {
...
class Calorimeter extends Detector {
