...
public class DoubleGaus{
    ...
    public DoubleGaus(double name, double min, double max, double peak1min, double peak1max, double peak2min, double peak2max, H1F histo) {
        this.name = name;
        this.min = min;
        this.max = max;
    ...
    public void fit(){
        ...
        f1.setParameter(1, histo.getxAxis().getBin((peak1max-peak1min)/2));
        f1.setParameter(2, (peak1max- peak1min)/4);

        f2.setParameter(0,histo.getBinContent(histo.getxAxis().getBin((peak2max-peak2min)/2)));
        f2.setParameter(1, histo.getxAxis().getBin((peak2max-peak2min)/2));
        ...
        for(int k = 0; k < 10; k++){
            ...
            f2.setParameter(0, param[3]);
            f2.setParameter(1, param[4]);
            f2.setParameter(2, param[5]);

            f3.setParameter(0, param[6]);
            f3.setParameter(1, param[7]);
            f3.setParameter(2, param[8]);
        }
    }
    ...
    public double getPeak1min() {
        return peak1min;
    }

    public double getPeak1max() {
        return peak1max;
    }

    public double getPeak2min() {
        return peak2min;
    }

    public double getPeak2max() {
        return peak2max;
    }
    ...
    public void setPeak1min(double peak1min) {
        this.peak1min = peak1min;
    }

    public void setPeak1max(double peak1max) {
        this.peak1max = peak1max;
    }

    public void setPeak2min(double peak2min) {
        this.peak2min = peak2min;
    }

    public void setPeak2max(double peak2max) {
        this.peak2max = peak2max;
    }

    public void setHisto(H1F histo) {
        this.histo = histo;
    }
}
